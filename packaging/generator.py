...
def main():
    ...
    if os.path.exists("packaging/pamplejuce.icns"):
        print("")
        print("Attach icns")
        subprocess.run(["mv", "packaging/pamplejuce.icns", "."])
        with open("tmpIcon.rsrc", "w") as outfile:
            subprocess.run(["echo", "read 'icns' (-16455) \"pamplejuce.icns\";"], stdout=outfile)
